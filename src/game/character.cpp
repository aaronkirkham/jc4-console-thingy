...
static const uint32_t HASH_SKIN_TINT_SETTING = "skin_tint_setting"_hash_little;
...
static void ChangeSkinImpl(CCharacter* character, const CRuntimeContainer* container)
{
    ...
    std::vector<std::vector<std::tuple<uint32_t, float, int32_t>>> model_infos;
    ...
    for (auto cont_ptr = container->begin(); cont_ptr != container->end(); ++cont_ptr) {
        ...
        if (rc.GetHash(HASH_CLASS_HASH) == HASH_CCHARACTER) {
            ...
            if (model_hash == 0) {
                    ...
                    if (sub_rc.GetHash(HASH_CLASS_HASH) == 0xC160E555) {
                        ...
                        for (auto b = sub_rc.begin(); b != sub_rc.end(); ++b) {
                            ...
                            if (sub_sub_rc.GetHash(HASH_CLASS_HASH) == 0xAA09A7DC) {
                                ...
                                int32_t tint_setting = sub_sub_rc.GetInteger(HASH_SKIN_TINT_SETTING);

                                model_infos[current_info_slot].push_back({model_hash, weight, tint_setting});
                                weight_sum += weight;
                            }
                        }
                ...
                model_infos[0].push_back({model_hash, 0.0f, 0});
            }
    ...
    for (size_t i = 0; i < model_infos.size(); ++i) {
        ...
        uint32_t model_hash   = std::get<0>(info[0]);
        int32_t  tint_setting = std::get<2>(info[0]);
        ...
        if (info.size() > 1) {
            ...
            *(uint32_t*)0x142C73A98 = random_seed;
            ...
            const auto rng = (((random_seed >> 16) | 0x3f8000) << 8);

            // choose a random model from the slots vector
            auto rng_weight = (*((float*)&rng) - 1.0f) * model_infos_weight_sum[i];
            for (const auto& slot_info : info) {
                const auto& [slot_hash, slot_weight, slot_tint_setting] = slot_info;

                rng_weight = (rng_weight - slot_weight);
                if (rng_weight <= 0.0f) {
                    model_hash   = slot_hash;
                    tint_setting = slot_tint_setting;
                    break;
                }
            }
        }
        ...
        // set the skin tint colour
        if (!model_state->m_setSkinTint && tint_setting) {
            model_state->m_setSkinTint = true;
            model_state->m_skinTint    = jc::CCharacterManager::instance().GetSkinTint(tint_setting);
        }
    }

    // get a random sub tint colour
    model_state->m_setSkinSubTint = true;
    model_state->m_skinSubTint    = jc::CCharacterManager::instance().GetRandomSkinSubTint();

    for (decltype(model_state->m_modelCount) i = 0; i < model_state->m_modelCount; ++i) {
        auto meta_data = CMetaEnvironmentalFX::Create();

        // @TODO: randomized tint colours for clothing
        // meta_data->m_tintColor1
        // meta_data->m_tintColor2
        // meta_data->m_tintColor3

        if (model_state->m_setSkinTint) {
            meta_data->m_skinTint = model_state->m_skinTint;
        }

        if (model_state->m_setSkinSubTint) {
            meta_data->m_skinSubTint = model_state->m_skinSubTint;
        }

        model_state->m_slot[i].m_modelInstance->SetMetaData(meta_data);
    }

    // @TODO: fix CSkinSwapper copying model parts for ^ this (problematic when switching rico skin within
