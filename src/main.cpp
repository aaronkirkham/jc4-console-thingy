...
BOOL WINAPI DllMain(HINSTANCE hinstDLL, DWORD fdwReason, LPVOID lpvReserved)
{
    ...
    if (fdwReason == DLL_PROCESS_ATTACH) {
        ...
        if (*(uint32_t *)0x141DFD9D0 != 0x6c617641) {
#ifdef DEBUG
        ...
        if (quick_start) {
            // quick start
            hk::put<bool>(0x142C286F8, true);

            // IsIntroSequenceComplete always returns true
            hk::put<uint32_t>(0x140E2CCB0, 0x90C301B0);

            // IsIntroMovieComplete always returns true
            hk::put<uint32_t>(0x140E2CC30, 0x90C301B0);
        }

        static hk::inject_jump<LRESULT, HWND, UINT, WPARAM, LPARAM> wndproc(0x140C1EC00);
        wndproc.inject([](HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam) -> LRESULT {
            auto game_state   = *(uint32_t *)0x142C286D0;
            auto suspend_game = *(bool *)0x142C2D348;
        ...
        static hk::inject_jump<void, jc::HDevice_t *> flip(0x140F33320);
